...
/// Covers TileRenderStateResolver, HexColorMapper.Dim, and the DefaultDimFactor constant.
...
public class FogOfWarRenderingTests
{
    ...
    // --- TileRenderStateResolver.Resolve ---
    ...
    // --- TileRenderStateResolver.DefaultDimFactor constant ---

    [Fact]
    public void Should_HaveDefaultDimFactorOfPointFive()
    {
        Assert.Equal(0.5f, TileRenderStateResolver.DefaultDimFactor);
    }
    ...
    public void Should_ProduceChannelsInValidRange_When_DimAppliedToTerrainColor()
    {
        foreach (var terrain in Enum.GetValues<TerrainType>())
        {
            ...
            (float R, float G, float B) dimmed = HexColorMapper.Dim(color, TileRenderStateResolver.DefaultDimFactor);
    ...
    public void Should_ReturnDimmerColor_When_DefaultDimFactorApplied()
    {
        ...
        var dimmed = HexColorMapper.Dim(color, TileRenderStateResolver.DefaultDimFactor);
    ...
    public void Should_ReturnHidden_When_OutOfBoundsCoordQueried()
    {
        ...
        // VisibilityMap.IsAt returns Unseen for out-of-bounds coords
